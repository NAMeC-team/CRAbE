...
fn avoid_allies_and_enemies(world: &World, obstacles: &mut Vec<Point2<f64>>, id: u8) -> Vec<Point2<f64>> {
...
impl Guard for PyRoulxsGuard {
    fn guard(&mut self, world: &World, commands: &mut CommandMap, _tools_commands: &mut ToolCommands) {
        ...
        commands.iter_mut().for_each(|(id, cmd)| {
            ...
            obstacles = avoid_allies_and_enemies(world, &mut obstacles, *id);
            avoid_ball(&world, &mut obstacles, &cmd);
            //dbg!(&obstacles);
            if let Some(rob_info) = world.allies_bot.get(&id) {
                //let rob_info = .unwrap(); // safe unwrap here
