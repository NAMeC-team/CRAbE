use crate::data::{CamBall, CamGeometry, CamRobot, FilterData, FrameInfo, TrackedBall, TrackedRobot, TrackedRobotMap};
use crate::PreFilter;
use chrono::{DateTime, LocalResult, TimeZone, Utc};
use crabe_framework::data::receiver::InboundData;
use crabe_framework::data::world::{AllyInfo, EnemyInfo, Robot, TeamColor};
use crabe_protocol::protobuf::vision_packet::{SslDetectionBall, SslDetectionFrame, SslDetectionRobot, SslGeometryData};
use log::error;
...
struct RobotDetectionInfo<'a> {
    detected_blue: &'a [SslDetectionRobot],
    detected_yellow: &'a [SslDetectionRobot],
    tracked_allies: &'a mut TrackedRobotMap<AllyInfo>,
    tracked_enemies: &'a mut TrackedRobotMap<EnemyInfo>,
}

fn detect_robots(detection: &mut RobotDetectionInfo, frame: &FrameInfo, team_color: &TeamColor) {
...
struct BallDetectionInfo<'a> {
    detected: &'a [SslDetectionBall],
    tracked: &'a mut TrackedBall,
}

fn detect_balls(detection: &mut BallDetectionInfo, frame: &FrameInfo) {
...
impl VisionFilter {
    ...
    fn handle_detection(
        ...
        let mut robot_detection_info = RobotDetectionInfo {
        ...
        detect_robots(&mut robot_detection_info, &frame_info, team_color);

        let mut ball_detection_info = BallDetectionInfo {
            detected: &detection.balls,
            tracked: &mut filter_data.ball,
        };

        detect_balls(&mut ball_detection_info, &frame_info)
    }

    fn handle_geometry(geometry: &SslGeometryData, filter_data: &mut FilterData) {
    ...
    fn step(
        ...
        inbound_data.vision_packet.iter().for_each(|packet| {
            if let Some(detection) = packet.detection.as_ref() {
                Self::handle_detection(detection, filter_data, team_color);
            }

            if let Some(geometry) = packet.geometry.as_ref() {
                Self::handle_geometry(geometry, filter_data);
            }
        });
    }
}
