...
impl UDPTransceiver {
    /// Creates a new non-blocking `UDPTransceiver` that listens on the
    ...
    /// # Returns
    ///
    /// A `Result` containing a new non-blocking `UDPTransceiver` that is ready
    /// to send and receive data if the operation is successful, or an
    /// `std::io::Error` if it fails.
    ///
    /// # Errors
    ///
    /// This function will return an `std::io::Error` if there is an error while
    /// binding and connect the socket or setting the socket to non-blocking
    /// mode.
    ///
    /// # Example
    ///
    ...
    ///
    /// let transceiver = UDPTransceiver::new(Ipv4Addr::LOCALHOST, 10301).expect("Failed to create UDPTransceiver");
    /// ```
    ///
    /// This example creates a new non-blocking `UDPTransceiver` that listens on
    /// the localhost IP address and port 10301, which is the default grSim
    /// control port for the blue team.
    ...
    /// # Arguments
    ///
    /// * `packet`: The packet to send. It should be a struct generated by
    ///   protobuf files using `prost` that implement the `prost::Message` and
    ///   `Default` traits.
    ///
    /// # Type Parameters
    ///
    /// * `T`: The type of the packet to send. It must implement the
    ///   `prost::Message` and `Default` traits. It's a struct generated by
    ///   protobuf files using `prost`.
    ...
    /// Attempts to receive a packet of type `U` from the socket, and decodes it
    /// using `prost`.
    ///
    /// # Type Parameters
    ///
    /// * `U`: The type of packet to decode. It must implement the
    ...
    /// successful and decoding is successful, or `None` otherwise.
