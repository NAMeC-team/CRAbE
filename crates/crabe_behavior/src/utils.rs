...
use crabe_math::geometry::frame;
use nalgebra::Isometry2;

pub fn pose_to_frame(pose: &Pose) -> Isometry2<f64> {
    frame(pose.position.x, pose.position.y, pose.orientation)
}
...
pub fn closest_bot_to_point<T>(bots: Vec<&Robot<T>>, point: Point2<f64>) -> Option<&Robot<T>> {
    bots.into_iter()
        .min_by(|robot1, robot2| robot1.distance(&point).total_cmp(&robot2.distance(&point)))
}
...
pub fn robots_to_circles<T>(world: &World, robots: Vec<&Robot<T>>) -> Vec<Circle> {
    robots
        .into_iter()
        .map(|robot| Circle::new(robot.position(), world.geometry.robot_radius))
        .collect()
}
...
pub fn object_in_bot_trajectory(
    world: &World,
    id: u8,
    target: Point2<f64>,
    ball: bool,
    ...
    let trajectory = Line::new(robot.position(), target);
    let mut objects = vec![];
    if ball {
    ...
    if ally {
        for (_, robot) in world.allies_bot.iter() {
            ...
            objects.push(Circle::new(robot.position(), world.geometry.robot_radius));
        }
    }
    if enemies {
    ...
    for object in objects {
        if trajectory.distance_to_point(&object.center)
            < world.geometry.robot_radius + object.radius
        {
...
pub fn ball_in_trajectory(world: &World, id: u8, target: Point2<f64>) -> bool {
    ...
    if let Some(ball) = &world.ball {
        let delta = 0.001;
        return trajectory.distance_to_point(&ball.position_2d())
            < world.geometry.robot_radius + world.geometry.ball_radius + delta;
    }
    false
}
...
fn get_obstruct_goal_zone_from_enemy(
    shoot_start_position: &Point2<f64>,
    enemy_position: &Point2<f64>,
    world: &World,
) -> Option<Line> {
    let start_pos_to_enemy = enemy_position - shoot_start_position;
    let perp = rotate_vector(start_pos_to_enemy.normalize(), PI / 2.)
        * (world.geometry.robot_radius + world.geometry.ball_radius + MARGIN_SHOOTING_WINDOW);
    let ray_left_side = (enemy_position + perp) - shoot_start_position;
    let ray_right_side = (enemy_position - perp) - shoot_start_position;
    let line_ray_left_side = Line::new(
        *shoot_start_position,
        shoot_start_position + ray_left_side * 1000.,
    );
    let line_ray_right_side = Line::new(
        *shoot_start_position,
        shoot_start_position + ray_right_side * 1000.,
    );
    let intersection_left =
        line_ray_left_side.intersection_segment_line(&world.geometry.enemy_goal.line);
    let intersection_right =
        line_ray_right_side.intersection_segment_line(&world.geometry.enemy_goal.line);
...
pub fn get_open_shoot_window(shoot_start_position: &Point2<f64>, world: &World) -> Vec<Line> {
    ...
    for enemy in world.enemies_bot.values() {
        if let Some(line) =
            get_obstruct_goal_zone_from_enemy(shoot_start_position, &enemy.position().xy(), world)
        {
...
pub fn get_best_shooting_window_bot<'a, T>(
    robots: &'a Vec<&Robot<T>>,
    world: &World,
) -> Option<&'a Robot<T>> {
    ...
    for robot in robots {
        ...
        if object_in_bot_trajectory(world, robot.id, robot_pos, false, false, true).len() > 0 {
            continue;
        }
        let shoot_windows = get_open_shoot_window(&robot_pos, world);
...
pub fn filter_robots_not_in_ids<'a, T>(
    robots: Vec<&'a Robot<T>>,
    ids: &Vec<u8>,
) -> Vec<&'a Robot<T>> {
    robots
        .into_iter()
...
macro_rules! require {
    ($expr:expr) => {
        match $expr {
            Some(value) => value,
            None => return $crate::behaviors::Status::Failure,
        }
    };
    ($expr:expr, $failure_state:expr) => {
        match $expr {
            Some(value) => value,
            None => return $failure_state,
        }
    };
}
